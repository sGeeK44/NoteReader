...
export const MusicScoreView = ({score, checker}: MusicScoreProps) => {
  ...
  const drawNote = (
    ...
    measure: number,
    note: number,
    result: 'TO_EARLY' | 'TO_LATE' | 'BAD_NOTE' | 'PERFECT' | 'START_POINT',
  ) => {
    const color = getColor(result);
    vexflowScore?.drawNote(svg, measure, note, color);
  };
  ...
  checker.onBadNote((measure: number, note: number, result: 'BAD_NOTE') => {
